/**
 * Beacon Pulse Engine v2
 * Improvements over v1:
 *   - Recency weighting (last 7 days = 3x weight)
 *   - Rapid Escalation signal
 *   - School baseline comparison
 *   - Trend shape classification
 *   - Improved session intensity (school day window)
 *   - Category clustering
 */
...
export function calculatePulseV2(studentId: string, events: BeaconEvent[]): StudentPulse {
  // Apply recency weighting to event scores
  const weightedEvents = events.map(e => ({ ...e, _weight: eventWeight(e) }));
...
export function calculateAllPulsesV2(events: BeaconEvent[]): StudentPulse[] {
  ...
  const pulses = Object.entries(byStudent)
    .map(([id, evts]) => calculatePulseV2(id, evts))
