...
function Sparkline({ data, color, width = 80, height = 28 }: { data: number[]; color: string; width?: number; height?: number }) {
  ...
  const lx     = width;
  const ly     = height - (data[data.length - 1] / max) * height;
  return (
...
const TREND_SHAPE_CONFIG = {
  sudden_spike:   { icon: "⚡", label: "Sudden Spike",   color: "text-red-600"    },
  gradual_climb:  { icon: "📈", label: "Gradual Climb",  color: "text-amber-600"  },
  chronic:        { icon: "⚠️", label: "Chronic",        color: "text-orange-600" },
  improving:      { icon: "📉", label: "Improving",      color: "text-emerald-600"},
  normal:         { icon: "✓",  label: "Normal",         color: "text-slate-400"  },
};
...
function StudentDetail({ pulse, events }: { pulse: StudentPulse; events: any[] }) {
  ...
  const shape  = TREND_SHAPE_CONFIG[pulse.trend_shape];
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              <span className="text-sm text-slate-400">{pulse.total_events} events</span>
              <span className="text-sm text-slate-400">
                First seen {new Date(pulse.first_seen).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
              </span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Pulse Score</div>
            <div className="text-6xl font-bold leading-none" style={{ color: alert.bar }}>{pulse.pulse_score}</div>
            <div className={`flex items-center justify-end gap-1 mt-1 text-sm font-semibold ${trend.color}`}>
              ...
              {pulse.trend_delta !== 0 && (
                <span className="text-xs font-normal text-slate-400 ml-1">
                  ({pulse.trend_delta > 0 ? "+" : ""}{pulse.trend_delta} vs prior 7d)
                </span>
              )}
            </div>
            {pulse.vs_school_avg !== undefined && (
              <div className="text-xs text-slate-400 mt-1">
                {pulse.vs_school_avg > 0
                  ? <span className="text-red-500 font-semibold">+{pulse.vs_school_avg} above school avg</span>
                  : pulse.vs_school_avg < 0
                  ? <span className="text-emerald-500 font-semibold">{pulse.vs_school_avg} below school avg</span>
                  : <span>At school average</span>}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Trend Shape",      value: <span className={shape.color}>{shape.icon} {shape.label}</span> },
              { label: "Last Incident",    value: new Date(pulse.last_seen).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) },
              { label: "Score vs Avg",     value: pulse.vs_school_avg !== undefined
                ? <span className={pulse.vs_school_avg > 0 ? "text-red-500" : "text-emerald-500"}>
                    {pulse.vs_school_avg > 0 ? "+" : ""}{pulse.vs_school_avg}
                  </span>
                : "—" },
              { label: "Total Events",     value: pulse.total_events },
            ].map((s, i) => (
        ...
        {pulse.categories.length > 0 && (
          <div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Concern Categories</div>
            <div className="flex flex-wrap gap-2">
              {pulse.categories.map(cat => (
                <div key={cat.name} className="flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2">
                  <span className="text-sm font-semibold text-slate-700">{cat.name}</span>
                  <span className="text-xs font-bold text-slate-400 bg-white px-1.5 py-0.5 rounded-full">{cat.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="rounded-2xl p-5 border-l-4" style={{ borderLeftColor: alert.bar, background: alert.light }}>
              ...
              return (
                <div key={sig.id} className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-lg font-bold" style={{ color: c }}>{sig.score}</span>
                  </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {studentEvents.map((event: any, idx: number) => {
              const rc = event.risk === "high" || event.risk === "critical" ? "#DC2626" : event.risk === "medium" ? "#F59E0B" : "#10B981";
              const bc = event.risk === "high" || event.risk === "critical" ? "border-red-200" : event.risk === "medium" ? "border-amber-200" : "border-slate-200";
              return (
                <div key={idx} className={`flex gap-4 p-4 rounded-xl border ${bc} bg-slate-50`}>
                  <div className="shrink-0 text-right w-20">
                    <div className="text-xs text-slate-400">{new Date(event.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div>
                    <div className="text-xs text-slate-400">{new Date(event.created_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</div>
                    <div className="text-xs font-bold mt-1" style={{ color: rc }}>{event.risk?.toUpperCase()}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {event.blocked && <span className="text-[10px] font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded-full">Blocked</span>}
                    </div>
                  </div>
                </div>
              );
...
export default function PulseBetaPage() {
  ...
  const schoolAvg = useMemo(() =>
    pulses.length ? Math.round(pulses.reduce((s, p) => s + p.pulse_score, 0) / pulses.length) : 0,
    [pulses]
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-700">Beta v2</span>
                {rapidCount > 0 && (
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-red-600 text-white animate-pulse">
                    ⚡ {rapidCount} Rapid Escalation{rapidCount > 1 ? "s" : ""}
                  </span>
                )}
              </div>
              <p className="text-sm text-slate-400 mt-0.5">
                Behavioural analytics · School avg: {schoolAvg} · {pulses.length} students
              </p>
            </div>
          </div>
          <Link
            ...
            return (
              <button key={level} onClick={() => setFilter(level)}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                  filter === level ? `${cfg.bg} ${cfg.text} ring-1 ring-offset-1` : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                }`}>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-72 shrink-0 bg-white border-r border-slate-200 overflow-auto">
            ...
            {!loading && filtered.map(pulse => {
              ...
              return (
                <button key={pulse.student_id} onClick={() => setSelected(pulse)}
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      {pulse.rapid_escalation && <span className="text-xs">⚡</span>}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs text-slate-400">{pulse.dominant_signal.label}</div>
                      <div className={`text-xs font-medium ${shape.color}`}>{shape.icon} {shape.label}</div>
                    </div>
                  ...
                  {pulse.vs_school_avg !== undefined && pulse.vs_school_avg > 10 && (
                    <div className="text-[10px] text-red-500 font-semibold mt-1">
                      +{pulse.vs_school_avg} above school avg
                    </div>
                  )}
                </button>
              );
